...
adb_timeout_multiplier="${FARMER_ADB_TIMEOUT_MULTIPLIER:-4}"
case "$adb_timeout_multiplier" in
    ''|*[!0-9]*) adb_timeout_multiplier=4 ;;
esac
if [ "$adb_timeout_multiplier" -lt 1 ]; then adb_timeout_multiplier=1; fi
if [ "$adb_timeout_multiplier" -gt 5 ]; then adb_timeout_multiplier=5; fi
adb_timeout() {
    adb_timeout_seconds="$1"
    shift
    timeout "$((adb_timeout_seconds * adb_timeout_multiplier))" "$@"
}
...
android_services_ready() {
    [ "$(adb_timeout 5 "$ADB_BIN" -s "$EMULATOR_SERIAL" shell getprop sys.boot_completed 2>/dev/null | tr -d '\r')" = "1" ] \
        || return 1
    adb_timeout 8 "$ADB_BIN" -s "$EMULATOR_SERIAL" shell cmd package list packages android 2>/dev/null \
        | tr -d '\r' | grep -qx 'package:android' \
        || return 1
    adb_timeout 8 "$ADB_BIN" -s "$EMULATOR_SERIAL" shell settings get global device_provisioned >/dev/null 2>&1 \
        || return 1
}

boot_timeout="${FARMER_ANDROID_BOOT_TIMEOUT_SECONDS:-600}"
...
while [ "$(date +%s)" -lt "$adb_root_deadline" ]; do
        adb_timeout 20 "$ADB_BIN" -s "$EMULATOR_SERIAL" root >/dev/null 2>&1 || true
        adb_timeout 20 "$ADB_BIN" -s "$EMULATOR_SERIAL" wait-for-device >/dev/null 2>&1 || true
        if [ "$(adb_timeout 10 "$ADB_BIN" -s "$EMULATOR_SERIAL" shell id -u 2>/dev/null | tr -d '\r')" = "0" ]; then
        adb_root_ready=1
        break
    fi
...
while [ "$(date +%s)" -lt "$timezone_deadline" ]; do
    adb_timeout 20 "$ADB_BIN" -s "$EMULATOR_SERIAL" shell \
        setprop persist.sys.timezone Asia/Taipei >/dev/null 2>&1 || true
    android_timezone="$(adb_timeout 10 "$ADB_BIN" -s "$EMULATOR_SERIAL" shell \
...
installed_apk_sha="$(adb_timeout 5 "$ADB_BIN" -s "$EMULATOR_SERIAL" shell cat /data/local/tmp/openpoint-apk.sha256 2>/dev/null | tr -d '\r\n' || true)"

if ! adb_timeout 8 "$ADB_BIN" -s "$EMULATOR_SERIAL" shell pm path ecowork.seven >/dev/null 2>&1 \
   || [ "$desired_apk_sha" != "$installed_apk_sha" ]; then
    ...
    while [ "$(date +%s)" -lt "$install_deadline" ]; do
        if adb_timeout 120 "$ADB_BIN" -s "$EMULATOR_SERIAL" install -r -d -g /opt/farmer/assets/openpoint.apk >/dev/null 2>&1; then
            install_ok=1
            break
        fi
...
adb_timeout 20 "$ADB_BIN" -s "$EMULATOR_SERIAL" shell am force-stop ecowork.seven
for _ in 1 2 3 4 5; do
    if ! adb_timeout 5 "$ADB_BIN" -s "$EMULATOR_SERIAL" shell pidof ecowork.seven \
...
adb_timeout 8 "$ADB_BIN" -s "$EMULATOR_SERIAL" shell settings put global window_animation_scale 0.0 >/dev/null 2>&1 || true
adb_timeout 8 "$ADB_BIN" -s "$EMULATOR_SERIAL" shell settings put global transition_animation_scale 0.0 >/dev/null 2>&1 || true
adb_timeout 8 "$ADB_BIN" -s "$EMULATOR_SERIAL" shell settings put global animator_duration_scale 0.0 >/dev/null 2>&1 || true
adb_timeout 8 "$ADB_BIN" -s "$EMULATOR_SERIAL" shell svc power stayon true >/dev/null 2>&1 || true
