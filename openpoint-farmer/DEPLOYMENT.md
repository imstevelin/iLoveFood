...
目前已驗證的封存檔索引同時包含 `linux/amd64` 與 `linux/arm64`。正式映像發布於 [Docker Hub](https://hub.docker.com/r/imstevelin/ilovefood-openpoint-farmer)，Docker 會依目標主機自動拉取正確架構；`2026.09.3` 是可重現的固定版本，`latest` 指向目前穩定版：

```bash
FARMER_IMAGE=imstevelin/ilovefood-openpoint-farmer:2026.09.3 \
...
skopeo copy --override-arch amd64 \
  ...
  docker-daemon:imstevelin/ilovefood-openpoint-farmer:2026.09.3
```
...
其他可調環境變數通常不需更改：`FARMER_TOKEN_REFRESH_SECONDS=180`、`FARMER_TOKEN_TTL_SECONDS=240`、Docker 版 `FARMER_FETCH_JOB_TIMEOUT_SECONDS=240`、`FARMER_ADB_TIMEOUT_MULTIPLIER=3`、`FARMER_API_WAIT_TIMEOUT_SECONDS=15`、`FARMER_MIN_HOST_AVAILABLE_MIB=128`。ADB 倍率只放寬容器內 Android 操作，不改變對外 API 與 Token 兑換驗證的 timeout。快取容量固定為 1，因為增加容量只會多做無必要的 App 查詢。
...
- fetch watchdog 獨立於 token worker；Docker 版預設 240 秒，舊 AVD 版預設 45 秒。即使 worker 卡在釋放 GIL 的原生函式，maintainer 仍能要求完整重建。
